...
def crevsig_to_dmg(crevSig, threshold_file, source,  img_res, wsize, tau_type='mean'):
    ...
    if 'mean' in threshold_dict.keys():  
        ...
        logger.info('-Selecting thresholds calculated for {}(crevSignal)'.format(tau_type))
    ...
    try: # check if runs with error (to handle old/new structuring of threshold.json files)
        ...
        logger.warning('Threshold could not be loaded: img_res[{}] / n_pix[''{}px''] combi not in dict for source {} \n' 
              '--> threshold & dmg set to None'.format(img_res,wsize,source))
        logger.warning('Main keys: ', threshold_dict.keys())
        threshold = None # set threshold to None
        dmg = None
    except: # other error
        logger.warning('Warning: Threshold could not be loaded for some reason. Set to None')
        threshold = None
        dmg = None
    else: # when 'try' succeeds, do the following:
        logger.info('--> Loaded  threshold {} --> calculate dmg'.format(threshold))
...
def read_img_to_grayscale(imPath, imName,dbmin=None, dbmax=None):
    ...
    if img_xyz.shape[2] > 3: # more than 3 bands. For now: assume that first three are RGB bands. Should update this.
        ...
        warnings.warn(f"Img has more than 3 bands (nbands={img_xyz.shape[2]}), assuming first three are RGB bands and selecting these.",UserWarning)
    ...
    if img_xyz.shape[2] == 3: # RGB band
        ...
        if dbmin is not None or dbmax is not None:
            ...
            logger.info(f'--> clip and normalise img using min,max: [{dbmin},{dbmax}]')
        ...
        # -- if no normalisation values are given, check if values are 0-255 RGB values
            ...
            logger.warning("img min max of [{:.1f},{:.1f}]: between [0 255] so assuming RGB values and proceeding with skimage.color.rgb2gray".format(img_xyz.min().values, img_xyz.max().values)) 
        ...
        if img_xyz.min().values < 0 or img_xyz.max().values>1:
            logger.warning(f'Found values of img outside bounds [0,1]: [{img_xyz.min().values:.1f}, {img_xyz.max().values:.1f}]')
            if img_xyz.quantile(0.9).values < 1 and img_xyz.quantile(0.1).values > 0:
                logger.warning(' --> 10 and 90th quantile are between [0,1] --> clip values to [0, 1]')
                img_gray = img_xyz.isel(band=0).clip(0,1)
            else:
                logger.info(f' --> 10 and 90th quantile are outside [0,1] --> clip values to [{dbmin}, {dbmax}]')
        ...
        raise ValueError(f'Unexpected image shape {img_xyz.shape} or {img.shape}; should have dimensions (y,x,band), with bands=1 or 3')
    ...
    logger.info(f'--> image succesfully converted to grayscale {img.shape} -->  {img_gray.shape}')
    return img_gray
...
def cut_img_to_windows(img_gray,wsize):
    ...
    logger.info('--> img of {} to {}'.format(img_gray.shape,windows.shape) )
