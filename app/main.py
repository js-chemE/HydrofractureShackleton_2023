...
if __name__ == '__main__':
    ...
    if mode == 0:
        """Testing Field"""
        metas = sgl.get_metadata_from_csv(settings.season_folder, ["tile-183", "tile-182", "tile-181"])
        coll = sgl.create_collection_from_meta(settings, metas)
        sgl.post_process_tifs(settings, coll, max_windows = 100, print_metadata = True, update_csv = True, update_tif = True)
        ...
        sgl.add_extent_statistics(settings.season_folder, coll, update_bool = True, print_bool = True)
        sgl.add_iceshelf_statistics(settings.season_folder, coll, update_bool = True, print_bool = True)
        ...
        files_tif, files_csv, files_shp, tile_names = sgl_utils.read_files_from_folder(settings.season_folder)
        sgl.process_and_export_csv(settings, files_csv, names = ["tile-183", "tile-182", "tile-181"])
        ...
        metas = sgl.get_metadata_from_csv(settings.season_folder, ["tile-183", "tile-182", "tile-181"])
        coll = sgl.create_collection_from_meta(settings, metas)
        files_tif, files_csv, files_shp, tile_names = sgl_utils.read_files_from_folder(settings.region_folder)
        ...
        metas = sgl.get_metadata_from_csv(settings.season_folder, ["tile-183", "tile-182", "tile-181"])
        coll = sgl.create_collection_from_meta(settings, metas)
        sgl.combine_filter_drainages(settings, coll, print_metadata = True, min_median_depth=0.5, min_std = 0.20)
