...
class Window:
    ...
    def file_dmg(self):
        if self.rangestart[0:4] < self.rangeend[0:4] and int(self.rangestart[4:6]) >= 8:
            ...
            self.rangestart[0:4] < self.rangeend[0:4] and int(self.rangestart[4:6]) < 8
        ):
...
class WindowCollection:
    ...
    def get_filtered_windows(
        self, names: list[str] = [], satellites: list[str] = [], dates: list[str] = []
    ) -> list[Window]:
        if names == []:
            names = self.names
        if satellites == []:
        ...
        return [
            ...
            if w.name in names
            and w.satellite in satellites
            and w.metadata["date"] in dates
        ]

    @property
    def delta_alpha_c_files(self) -> List[str]:
        delta_alpha_c_files = list(
            set([str(w.file_delta_alpha_c) for w in self.windows])
        )
    ...
    def from_settings(cls, settings):
        return cls(
            name=settings.region.lower()
            + settings.rangestart[2:4]
            + settings.rangeend[2:4],
...
def get_angle(gdf: gpd.GeoDataFrame) -> float:
    p1 = gdf["geometry"].exterior[0].coords[1]  # type: ignore
    p2 = gdf["geometry"].exterior[0].coords[2]  # type: ignore
...
def create_active_crevasses_mask(d_alpha, target, spread):
    ...
    active_mask[(d_alpha >= target - spread) & (d_alpha <= target + spread)] = d_alpha[
        (d_alpha >= target - spread) & (d_alpha <= target + spread)
    ]
    active_mask[(d_alpha <= -target + spread) & (d_alpha >= -target - spread)] = (
        d_alpha[(d_alpha <= -target + spread) & (d_alpha >= -target - spread)]
    )
    active_mask[
        (d_alpha >= 180 - target - spread) & (d_alpha <= 180 - target + spread)
    ] = d_alpha[(d_alpha >= 180 - target - spread) & (d_alpha <= 180 - target + spread)]
    active_mask[
        (d_alpha <= -180 + target + spread) & (d_alpha >= -180 + target - spread)
    ] = d_alpha[
        (d_alpha <= -180 + target + spread) & (d_alpha >= -180 + target - spread)
    ...
    high[abs(active_crevasses) >= 180 - target - spread] = (
        abs(active_crevasses[abs(active_crevasses) >= 180 - target - spread])
        - 180
        + target
    )
    low = np.zeros(active_crevasses.shape)
    low[abs(active_crevasses) <= target + spread] = (
        abs(active_crevasses[abs(active_crevasses) < target + spread]) - target
    )
...
def read_files_from_folder(
    ...
    try:
        ...
        names = []
    print(
        f"     | -> {len(files_tif)} (.tif), {len(files_csv)} (.csv) and {len(files_shp)} (.shp) for {len(names)} names"
    )
    print(f"     | -> {names}")
